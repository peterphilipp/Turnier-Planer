...
export interface WorkArea { id: number; name: string; icon: string; order?: number; color: string; minVolunteers: number; maxVolunteers: number; isStandard?: boolean; categories?: WorkAreaCategory[]; }
...
export interface Volunteer { id: number; name: string; email: string | null; phone: string | null; role?: 'HELPER' | 'ORGANIZER' | 'ADMIN'; tournamentId: number | null; consentGiven?: boolean; consentDate?: string; createdAt?: string; lastLoginAt?: string | null; lastActivityAt?: string | null; children?: { childName: string; childYear: number }[]; pushSubscriptions?: { id: number; userAgent?: string | null; createdAt?: string; deviceLabel?: string }[]; }
